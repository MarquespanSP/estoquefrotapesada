...
ALTER TABLE maintenance_uploads ENABLE ROW LEVEL SECURITY;

-- Políticas para manutenções
CREATE POLICY "Enable read access for authenticated users" ON maintenances
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Enable insert for authenticated users" ON maintenances
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Enable update for authenticated users" ON maintenances
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable delete for authenticated users" ON maintenances
    FOR DELETE USING (auth.role() = 'authenticated');

-- Políticas para itens de manutenção
CREATE POLICY "Enable read access for authenticated users" ON maintenance_items
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Enable insert for authenticated users" ON maintenance_items
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Enable update for authenticated users" ON maintenance_items
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable delete for authenticated users" ON maintenance_items
    FOR DELETE USING (auth.role() = 'authenticated');

-- Políticas para uploads
CREATE POLICY "Enable read access for authenticated users" ON maintenance_uploads
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Enable insert for authenticated users" ON maintenance_uploads
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Enable update for authenticated users" ON maintenance_uploads
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable delete for authenticated users" ON maintenance_uploads
    FOR DELETE USING (auth.role() = 'authenticated');
